...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		...
		// 1 строка — title, 1 пустая после него,
		// 1 строка — заголовок OTHER, 1 пустая после таблицы OTHER,
		// 1 строка — заголовок TG
		// Итого минимум 5 служебных строк + по 1 рамочная строка на каждую таблицу.
		// Возьмём безопасный запас:
		const chrome = 6
	...
	case PacketMsg:
		m.updateStat(msg)
		// Не дергаем refresh на каждый пакет — перерисовываем по тику
...
func (m *model) updateStat(packet PacketMsg) {
	m.total++
	if _, ok := m.perIP[packet.IP]; !ok {
		m.ipOrder = append(m.ipOrder, packet.IP)
		m.perIP[packet.IP] = &ipStat{
			count: 1,
			last:  packet.T,
			proto: packet.Proto,
			isTG:  m.tgcidr != nil && m.tgcidr.Contains(packet.IP),
		}
		return
	}
	st := m.perIP[packet.IP]
	st.count++
	st.last = packet.T
	st.proto = packet.Proto
}

/* ==== разбор на таблицы и сортировка ==== */
...
func (m *model) splitAndSortIPs() (tgIPs, otherIPs []string) {
	...
	// Сортировка по count (desc), при равенстве — по last (новее выше)
...
/* ==== динамические ширины для обеих таблиц (общие) ==== */

func (m *model) colWidthsForBoth(tgIPs, otherIPs []string) []int {
	// старт — с длин заголовков
	wIP := len("IP")
	wPkts := len("Пакеты")
...
func (m *model) refreshTables() {
	...
	// Строки
	tgRows := m.rowsFromIPs(tgIPs)
	otherRows := m.rowsFromIPs(otherIPs)

	// Обновляем таблицы
	m.tgTable.SetColumns(cols)
	m.tgTable.SetRows(tgRows)

	m.otherTable.SetColumns(cols)
	m.otherTable.SetRows(otherRows)

	// Стили один раз применяем (или при каждом refresh — не страшно)
	st := table.Styles{
...
func main() {
	...
	localIP, _ := network.GetLocalIP("en0")

	m := model{
		pick:       pickRemote,
		events:     reader.Events(),
		localIP:    localIP,
		tgcidr:     telegram.LoadIP(),
	...
	// стартовая инициализация таблиц
	m.refreshTables()
